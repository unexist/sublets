...
class DBus # {{{
  ...
  def ary2ptr(ary)
    ...
    ary.each do |s|
      pary << FFI::MemoryPointer.from_string(s)
    end

    ptr = FFI::MemoryPointer.new(:pointer, pary.size)
    pary.each_with_index do |value, i|
      ptr[i].put_pointer(0, value)
    end
...
configure :dbus do |s|
  ...
  # Create window
  geom   = Subtlext::Screen[0].geometry #< Get screen geometry
  width  = 150
  height = 40
  
  s.win = Subtlext::Window.new("dbus", [ 
      geom.x + (geom.width - width) - 10, 
      geom.y + (geom.height - height) - 10, width, height 
    ], 
    3, COLORS[:fg_focus], COLORS[:bg_panel], COLORS[:border_focus]
  )

  # Watch socket and hide sublet from panel
  s.watch(s.dbus.fd)
  s.hide
end

on :run do |s|
  # Just hide window and reset interval
  s.interval = 9999
  s.win.hide
end
...
on :watch do |s|
  if(s.dbus.fetch)
    ...
    s.win.show
    s.win.puts(data, 10, 10)

    # Set interval to hide window
    s.interval = case(s.dbus.timeout)
      when -1 then 2
      when 0  then 0
      else
        (s.dbus.timeout / 1000) - 1
    end
  end
end
